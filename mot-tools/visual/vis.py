...
def plot_pose(im, all_pose_data):
    ...
    with open(all_pose_data, 'r') as f:
        for line in f.readlines():
            ...
            for i in range(17):
                i = 3*i
                v = int(pose[i+2])  
                x = int(pose[i]) if int(pose[i+2])>0 else 0
                x = x if int(pose[i])<im.shape[1] else im.shape[1]
                y = int(pose[i+1]) if int(pose[i+2])>0 else 0
                y = y if int(pose[i+1])<im.shape[0] else im.shape[0]
                if v != 0:
                    cv2.circle(im, (x,y), 1, (255, 0, 0), 4)
                pointpairs.append((x, y))
                flag.append(v)
            for j, e in enumerate(EDGES):
                if flag[int(e[0])-1] in disable or flag[int(e[1])-1] in disable:
                    continue
                cv2.line(im,
                        pointpairs[int(e[0])-1],
                        pointpairs[int(e[1])-1],
                        EC[j],
                        2,
...
# images dict
def polt_vfusion(images,unit_size=(1920, 1080), rows=1, pads=10):
